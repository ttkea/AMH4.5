...
StartDateSecond='';
PHPDisable='';
Base_url='http://down.awkxy.com/webpanel/amh425/';
...
Php54Version='php-5.4.44';
Php55Version='php-5.5.28';
Php56Version='php-5.6.12';
Php70Version='php-7.0.0';
...
function Uninstall()
{
	...
	rm -rf /usr/local/php/ /usr/local/php5.4/ /usr/local/php5.5/ /usr/local/php5.6/ /usr/local/php7.0/ /usr/lib/php /etc/php.ini /etc/php.d /usr/local/zend;
	rm -rf /home/wwwroot/;
...
function InstallLibiconv()
{
	...
	Downloadfile "${LibiconvVersion}.tar.gz" "${Base_url}${LibiconvVersion}.tar.gz";
...
function InstallOpenSSL()
{
	...
	Downloadfile "${OpenSSLVersion}.tar.gz" "${Base_url}${OpenSSLVersion}.tar.gz";
...
function InstallMysql()
{
	...
	Downloadfile "${MysqlVersion}.tar.gz" "${Base_url}${MysqlVersion}.tar.gz";
...
function InstallPhp()
{
	...
	Downloadfile "${PhpVersion}.tar.gz" "${Base_url}${PhpVersion}.tar.gz";
...
function InstallPhp54()
{
	...
	Downloadfile "${Php54Version}.tar.gz" "${Base_url}${Php54Version}.tar.gz";
...
function InstallPhp55()
{
	...
	Downloadfile "${Php55Version}.tar.gz" "${Base_url}${Php55Version}.tar.gz";
...
function InstallPhp56()
{
	...
	Downloadfile "${Php56Version}.tar.gz" "${Base_url}${Php56Version}.tar.gz";
	...
	if [ ! -d /usr/local/php5.6 ]; then
		...
		if [ "$InstallModel" == '1' ]; then
			./configure --prefix=/usr/local/php5.6 --enable-fpm --with-fpm-user=www --with-fpm-group=www --with-config-file-path=/usr/local/php5.6/etc --with-config-file-scan-dir=/etc/php.d/5.6 --with-openssl --with-zlib  --with-curl --enable-ftp --with-gd --with-jpeg-dir --with-png-dir --with-freetype-dir --enable-gd-native-ttf --enable-mbstring --enable-zip --with-iconv=/usr/local/libiconv --with-mysql=mysqlnd --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --enable-opcache --without-pear $PHPDisable;
		fi;
		make -j $Cpunum;
		make install;
...
function InstallPhp70()
{
	...
	Downloadfile "${Php70Version}.tar.gz" "${Base_url}${Php70Version}.tar.gz";
...
function InstallNginx()
{
	...
	Downloadfile "${NginxVersion}.tar.gz" "${Base_url}${NginxVersion}.tar.gz";
	Downloadfile "${NginxCachePurgeVersion}.tar.gz" "${Base_url}${NginxCachePurgeVersion}.tar.gz";
...
function InstallPureFTPd()
{
	...
	Downloadfile "${PureFTPdVersion}.tar.gz" "${Base_url}${PureFTPdVersion}.tar.gz";
...
function InstallAMH()
{
	...
	Downloadfile "${AMHVersion}.tar.gz" "${Base_url}${AMHVersion}.tar.gz";
...
function InstallAMS()
{
	...
	Downloadfile "${AMSVersion}.tar.gz" "${Base_url}${AMSVersion}.tar.gz";
